...
async def check_inventory(product_id: str, quantity: int):
    ...
    if product["stock"] >= quantity:
        ...
        return {"id": product_id, "available": True, "stock": new_stock}
    logger.info(f"Checked inventory for product {product_id}: available=False")
    return {"id": product_id, "available": False, "stock": product["stock"]}
